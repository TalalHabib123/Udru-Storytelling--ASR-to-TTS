...
MODEL_TYPE = "arabic"  # Default model type for audio synthesis: hindi, latin, arabic
...
def translate_text_to_audio(text, file_id):
    ...
    """
    try:
        inputs = tokenizer(text, return_tensors="pt").to(get_device())  # Move inputs to the same device as the model
        ...
        scipy.io.wavfile.write(f"{file_path}",
                               ...
                               data=output.squeeze().cpu().numpy())  # Move output to CPU before converting to numpy
